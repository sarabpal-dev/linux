...
#define M1120_VDD_TPY_UV	(3000000)

#define M1120_VIO_MIN_UV	(1650000)
#define M1120_VIO_MAX_UV	(3600000)
#define M1120_VIO_TPY_UV	(1800000)

#define HALL_MXM1120_UP "oplus,hall-mxm1120,up"

#define SIGNED_MASK			(0x10000)

#define TRI_KEY_TAG                  "[tri_state_key:up] "
...
static int m1120_power_init(struct m1120_data_t *data)
{
	...
	if (regulator_count_voltages(data->vdd) > 0) {
		if ((data->vdd_volt == 0) || (data->vdd_volt < M1120_VDD_MIN_UV)
			|| (data->vdd_volt > M1120_VDD_MAX_UV)) {
			ret = regulator_set_voltage(data->vdd, M1120_VDD_MIN_UV, M1120_VDD_MAX_UV);
		} else {
			ret = regulator_set_voltage(data->vdd, data->vdd_volt, data->vdd_volt);
		}
		if (ret) {
	...
	if (regulator_count_voltages(data->vio) > 0) {
		if ((data->vio_volt == 0) || (data->vio_volt < M1120_VIO_MIN_UV)
			|| (data->vio_volt > M1120_VIO_MAX_UV)) {
			ret = regulator_set_voltage(data->vio, M1120_VIO_MIN_UV, M1120_VIO_MAX_UV);
		} else {
			ret = regulator_set_voltage(data->vio, data->vio_volt, data->vio_volt);
		}
		if (ret) {
		...
		struct m1120_data_t *p_data)
{
	...
	if (rc) {
		...
		p_data->data_offect = 0;
		if (temp_val > 0) {
			mask = temp_val & SIGNED_MASK;
			if (mask) {
				mask = -1;
			} else {
				mask = 1;
			}
			p_data->data_offect = (temp_val & (SIGNED_MASK - 1)) * mask;
		}
		TRI_KEY_ERR("get data_offect:%d\n", p_data->data_offect);
	}

	temp_val = 0;
	rc = of_property_read_u32(np, "vdd_volt", &temp_val);
	if (rc) {
		TRI_KEY_ERR("not get vdd voltage from dts.\n");
		temp_val = 0;
	}
	p_data->vdd_volt = temp_val;
	temp_val = 0;
	rc = of_property_read_u32(np, "vio_volt", &temp_val);
	if (rc) {
		TRI_KEY_ERR("not get vio voltage from dts.\n");
		temp_val = 0;
	}
	p_data->vio_volt = temp_val;
	TRI_KEY_LOG("vdd_volt : %u, vio_volt: %u.", p_data->vdd_volt, p_data->vio_volt);

	p_data->irq_en = of_property_read_bool(np, "magnachip,use-interrupt");

	p_data->esd_flag = false;
	p_data->esd_flag = of_property_read_bool(np, "esd-check-support");
...
{
	TRI_KEY_LOG("%s:%d", __func__, enable);
	...
	if (enable) {
		if (p_m1120_data->irq_enabled) {
			TRI_KEY_ERR("IRQ is already enable.\n");
			return 0;
		} else {
		...
		if (p_m1120_data->irq_enabled) {
			...
			TRI_KEY_ERR("IRQ is already disable.\n");
		}
	}
...
static void m1120_dump_reg(struct seq_file *s)
{
	int i, err;
	u8 val;
	u8 buffer[512] = {0};
	...
	if (mode & DETECTION_MODE_INTERRUPT) { /*interrupt mode*/
		if (!p_m1120_data->irq_enabled) {
			...
			if (request_irq(p_m1120_data->irq, &m1120_up_irq_handler,
				IRQ_TYPE_EDGE_FALLING | IRQF_ONESHOT, "hall_m1120_up",
...
static int offect_data_handle(int offect)
{
	int ret = 0;

	if (offect == READ_OFFECT) {
		ret = p_m1120_data->data_offect;
		TRI_KEY_ERR("UP->%s:READ OK [%d][%d]",
			__func__, offect, ret);
	} else {
		p_m1120_data->data_offect = offect;
		TRI_KEY_ERR("UP->%s:WRITE OK [%d][%d]",
...
static int tri_key_m1120_i2c_drv_probe(struct i2c_client *client)
{
	...
	if (p_data->esd_flag == true) {
		TRI_KEY_LOG("enabel esd check\n");
		hall_dev->enable_esd_check = true;
	}
	...
	p_m1120_data->source = wakeup_source_register(NULL, "m1120 wakeup source hall down");
	if (p_m1120_data->source == NULL) {
		TRI_KEY_ERR("m1120 wakeup source hall down register fail \n");
	}
...
static void m1120_i2c_drv_remove(struct i2c_client *client)
#else
