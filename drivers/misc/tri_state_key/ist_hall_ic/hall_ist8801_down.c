...
static void ist8801_dump_reg(struct seq_file *s)
{
	int i, err;
	u8 val;
	u8 buffer[512] = {0};
	...
	if (on) {
		if (regulator_count_voltages(ist8801_data->power_2v8) > 0) {
			ret = regulator_set_voltage(ist8801_data->power_2v8,
					2856000, 3104000);
			if (ret) {
				TRI_KEY_LOG("Regulator failed vdd[%d]\n", ret);
				return ret;
			}
 ...
 */
static bool ist8801_down_update_threshold(int position,
...
static int ist8801_reset_device(struct ist8801_data_t *ist8801_data)
{
	...
	if (data != IST8801_VAL_DID) {
		TRI_KEY_ERR("current device id(0x%02X), is not IST8801 device id(0x81)\n", data);
	}
...
static int ist8801_i2c_probe(struct i2c_client *client)
{
	...
	g_ist8801_data->source = wakeup_source_register(NULL, "ist8801 wakeup source hall down");
	if (g_ist8801_data->source == NULL) {
		TRI_KEY_ERR("ist8801 wakeup source hall down register fail\n");
	}
...
static void ist8801_i2c_remove(struct i2c_client *client)
{
}
#else
